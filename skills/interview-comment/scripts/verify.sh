...
while IFS= read -r d; do
  ...
  if [[ $ok_ev -eq 1 ]]; then
    ...
    grep -qE 'coding-analysis\.md|review-log\.md|review-(sol|terra|luna)|取样单元|压分项|前置门|(references/|本 ?skill 的 ?|见 ?)SKILL\.md|判分依据|取样缺口|未取样|补测|决策表|期望线只要求|按校招期望线判|按社招期望线判|按校招线判|落 ?[0-9.]+\\+? ?而非|规则明文|命中决策表|按规则第' "$ev" \
      && gate+="含内部术语或断链引用 "
    ...
    # ---- 自述"两条独立负向"却填 = （事故 2026-09-12，第四次同形态）----
    # 规则第 2 条：无 N2 但有 >=2 条相互独立的 N1 → 判 -。
    # 连续四次犯同一个错：正文如实写了两条独立负向,维度表却填 =。
    # 最严重的一次是 liuxinqi：留痕里已写明"这已是 3 条相互独立的 N1、第 2 条本该
    # 命中",却用"他确实自主定义了指标体系(两条独立 P)"把它按回 = ——
    # **禁止正负相抵这条在符号生成层同样适用,不只在综合分层**。
    # 判定：维度依据段里某维自述"两处/两条(相互)独立"的欠缺,该维符号不得是 =。
    # ⚠️ 编码与算法维度不纳入本检查：规则明文「代码写不完、实测不通过、实现 bug、
    # 零自测、复杂度推不出闭式」都不算 N1/N2,C2 的符号按分析过程定。把编码的
    # 「完成度欠缺」当 N1 计数会误报(实测 liuxinqi 就被误拦)。
    awk '/^### 维度依据/,/^### 优点/' "$ev" \
      | grep -E '^\*\*(业务理解|技术支撑|技术广度|技术深度|软素质) =\*\*|^(业务理解|技术支撑|技术广度|技术深度|软素质) `=`' \
      | grep -qE '两处相互独立|两条相互独立|两处独立的欠缺|两条独立的欠缺|两处独立缺口|两处核心缺口|[两三四五]处(关键)?[^，。]{0,8}(站不住|不掌握|答不出|缺失|欠缺|缺口)|[两三四五]条(相互)?独立' \
      && gate+="某维自述多条独立负向却填=(规则第2条应判-) "

    # ---- 校招场必须按校招期望线判，不能只在概览写基准（事故 2026-09-12）----
    # 事故：某 27 届在读本科生，概览写了"对标 1-1 校准",实际打分却用社招标准——
    # 拿"成本 ROI 算不出""并发语义类比不精确"压了技术支撑,把"没关注业务细节"
    # 判成业务理解负向。面试官指出"不能拿校招的标准跟社招的标准对比"后,技术支撑
    # 由 - 改判 +、业务理解由 - 改判 =,综合分 2 → 2.5。
    # 根因："写了校准基准不等于按它执行了"——与"写了归属说明却仍给旁听场打档"同类。
    # 判定：概览声明按校招校准时,**review-log.md**(内部留痕)里必须出现按校招线判定
    # 的痕迹,逼作者逐条对照那五条而不是套默认标准。
    # ⚠️ 2026-09-14 调整落点：原先查 evaluation.md,但"按校招期望线判""校招期望线
    # 只要求…"这类是内部评分机制表述,属过程信息,不该进对外交付物(用户明确指出)。
    # 判定过程留在 review-log,对外只写候选人表现与结论。
    # 例外:登记为校招但实际已毕业在职的场次(报告里会写明该矛盾),不触发。
    # 触发：概览结论句（含"综合评分"的那一行）里出现"校招"
    if grep -E '综合评分' "$ev" | grep -q '校招' \
       && ! grep -qE '已毕业|在职约|与简历显示的' "$ev"; then
      rl="$d/review-log.md"
      if [[ -s "$rl" ]]; then
        grep -qE '按校招|校招期望线|校招线|校招标准' "$rl" \
          || gate+="声明按校招校准但review-log无校招线判定痕迹 "
      fi
    fi
    ...
    if [[ "$ov_chars" -gt 400 ]]; then
      gate+="概览${ov_chars}字超400上限 "
    fi
