...
4. **一条反馈含多个问题时逐个独立判定**:用户一条投诉里常并列好几个现象(如"设置失效 + 打不开项目 + 黑屏 + 页面报错")。**禁止合并成一个 verdict**——它们的归属模块、根因、当前状态往往完全不同,典型分布是「一个真 bug、一个复现不出、一个是用户自身状态被误当故障、一个早已修好」。做法:
   - 抽取阶段就把现象拆成编号子问题,每条独立走证据链与打分,产出各自的 verdict;
   - 报告里一条一行,**明确标注"已修 / 未修 / 无法复现 / 非缺陷"**,并给出各自依据;
   - 用户只要求看其中一条时(「只关注 X」),先按要求聚焦答清 X,其余子问题列出结论一句话带过,不要擅自扩大战场。
5. **陈旧反馈先做时效性核查**:反馈距今已久(如一个月前)时,第一动作是核对**这段时间内有没有已合入的相关修复**——按现象关键词查提交/PR 与上线时间,与投诉时间对齐。已修的直接标"已修 + 修复出处 + 上线时间",不要重跑一遍完整定位;仍未修的才进 Step 7。跳过这一步会花大量算力实锤一个早就修好的问题。
6. **反馈载体(文档 / 截图 / 长贴)是一等输入,压缩后必须重读原件而不是凭摘要作答**:用户给的往往不是一句话,而是一份列了多条问题的协作文档或一张界面截图。踩过:上下文压缩后按摘要里残留的印象答题,把用户反馈的问题与自己排查中发现的问题混成一锅,被当场质疑「我给你发的是一个文档,你结合整个 session 的上下文给我说清楚……**你是失忆了吗?你好好看清楚**」。规则:
   - 原件路径 / 链接落在草稿里(文档 URL、截图存放路径),恢复上下文后的第一动作是**回读原件**,不是复述摘要;摘要只用于定位,不作为事实来源。
   - **"用户反馈的问题"与"我自己发现的问题"必须分列两张表**,各自标来源。用户会明确要求区分这两类("我发现的问题是什么?用户反馈的问题是什么?"),混在一起等于让他无法判断哪条需要他去回复反馈方。
   - 载体里列了 N 条就必须交付 N 行,每行按「是否属实 / 到底是什么问题 + 根因 / 怎么解决」三段回答,并各自附证据来源(日志、线上数据、代码、文档)。少一条、或几条并成一条,都会被打回。
   - 数值型断言优先追问"这个值是谁设的":用户报的阈值/上限/配额类现象(如某项下限异常),先分清**是用户自己设置的、系统默认的、还是代码推算出来的**——三者的处置完全不同(不是缺陷 / 配置问题 / 真 bug)。结合代码、文档、线上日志三方交叉后再给结论。
...
3. **按 module 字段决定候选仓**(至少 1 个,可多个并开):
   ...
   - `module=recsys` → 独立推荐仓
   - `module=ios/android/web` → 对应客户端仓。**双端同时爆发是关键路由信号**:iOS 和 Android 同一时间窗出现同类投诉 → 缺陷大概率在共享层(跨端 RN/热更 Bundle、共享 JS 状态、后端),单端独有 → 原生层。前端仓要查的不是接口而是**状态机**:该模式/视图态谁写入(grep setXxx)、谁清除、清除路径上的 UI 元素依赖什么资产(图标丢失=路径断裂)、状态是否持久化(持久化则重启不自愈)
...
**扩样取证的收手判据(P1 规则,来自一次"扩了几轮样一个实例都没有"复盘)**:第一批取证没找到实例时,正确动作是**先判断缺的是样本量还是判定口径**,再决定扩不扩:
...
**召回方法本身要先被验证(P0 规则,来自一次"找不到实例是你的方法问题"复盘)**:靠关键词/正则命中去捞"用户有没有表达某种体验问题"(抱怨、困惑、不满、觉得被忽略)几乎必然召回不到——这类现象藏在语义里而不在词面上,用户极少用你猜的那个词。这种召回方式下的"扫了一遍没命中"不构成任何证据,用户的纠偏形态是「找不到是你的方法问题,不应该用正则匹配或关键词命中,应该用语义理解」。规则:
...
**举证的确认偏差门禁(P0 规则,来自一次"你找的还是正样本"复盘)**:找到几条支持结论的样本不等于结论成立。要下"这个现象存在且有影响"的判断,必须同时交出反面那一半,否则整条结论上限只能是 hypothesis:
...
**统计口径必须自带三件套(P1 规则,来自一次"这几个百分比分别是怎么算的"追问复盘)**:报告里每出现一个比例或群体规模,都会被逐个追问「这批人是怎么筛出来的、分子分母各是什么、两个比例之间是不是子集关系」。答不出任一项,这个数字就不该写进结论。每个数字紧邻处交代:
...
**自造指标名必须就地定义**:排查过程里临时起的分层名、率名、余量名(形如 `<某层>率`、`<某判据>成立率`、`margin`)对读者是黑话,用户会逐个问"这是什么含义,解释一下"。规则:第一次出现时用一句话给出**定义 + 计算方式 + 该值大小说明什么**;能换成自解释的普通说法就别自造名字。
...
**"有 owner"≠"有 CS 口径"(P1 规则,来自"cinema 无法退出"真实事故)**:owner 在群里给的是**内部技术结论**("重启拉新 Bundle"),不是可发用户的话术——两者中间隔着:①操作步骤具体化(哪个按钮、什么顺序、失败后怎么办;技术结论漏掉的步骤要从前端代码补全,如"重启后还必须手动点退出钮清持久化状态");②失败兜底(重启两次无效→回收 uid+录屏升级);③中英对照。报告发现群里只有技术结论没有用户话术时,把"补 CS 口径"作为本组 deliverable,不要假设 CS 自己会翻译。
...
### 主数据存储捞不到/导不出时的证据获取降级(P1 规则,来自一次取数受阻复盘)
...
### 跨存储脏数据清理的完整性门禁(P0 规则,来自多次"清一半留悬空引用"复盘)
...
### 批量清理的"选中依据"门禁(P0 规则,来自多次"这 N 条你凭什么判定是脏的"追问复盘)
...
### 日志抽取的正则安全(P0 规则,来自一次把共享开发机内存打爆的事故)
...
### 机制归因的四道门禁(P0 规则,来自一次热更产物损坏事故的根因误判复盘)
...
### 跨服务同名字段的边界门禁(P0 规则,来自一次"这个接口到底返回什么"连错两轮的复盘)
...
- **飞书推送只发给配置的用户本人**(`my_open_id` 私聊)——未经用户当次明确授权,不发任何群、不发第三人。用户原话「以后只允许给我自己发消息,这是纪律」;发错一次即事故级
