...
3. **是否有「代码考核」平台产物** → 决定 0.4 做不做（**正向信号**）。在候选人页/评价区找「代码考核」卡片是否存在：
   - 存在 → `coding=yes`
   - 不存在 → `coding=no`

> ⚠️ **`coding=no` 只代表"没有平台代码产物"，绝不等于"本轮没考编程题"。** 屏幕共享出题、白板、本地 IDE、纯口述都不会产生「代码考核」卡片。**实测两场面试探针均返回 `coding=no`，而两场都用屏幕共享出了算法题——漏判率 2/2。** 若据此按"本轮未考"处理，会把候选人读错题、剪枝答不全、只口述没落代码这些关键信号整段漏掉。
>
> **所以判"本轮考没考编程题"必须两路都走**：
> 1. 探针 `coding=yes` → 走 0.4 捞平台代码原文与运行记录；
> 2. **无论探针是 yes 还是 no，都要回 `asr.md` 扫一遍编程题信号**（关键词：`算法题`/`看一个题`/`看下这个题`/`共享一下`/`写一下`/`编程题`/`leetcode`）。扫到就说明考了，按 scoring-rubric「传统算法/编程题专项评估」的形态表定性（多为"只口述、没落代码"），在 `evaluation.md`「编程题」一节如实记录，**不得写"本轮未考"**。
>
> 换句话说：`coding` 字段回答的是"有没有代码产物"，`asr.md` 回答的是"有没有考"。**两个问题分开判，不要用前者代替后者。**
>
> ⚠️ 另注：**不要**用 `meeting_coding_status` 之类"会中状态"接口判定（它会后返回 `is_coding_enabled:false` 不代表没编程记录，曾据此误判、下修一整档，见 0.4）。
...
> 前置：只有 0.15 探针判 `coding=yes`（「代码考核」卡片存在）才进入本步捞**平台产物**；`coding=no` 则跳过本步。
>
> ⚠️ **但跳过本步 ≠ 本轮没考编程题。** 按 0.15 第 3 条，`coding=no` 时仍须回 `asr.md` 扫编程题信号；扫到则说明是屏幕共享/白板/口述出题（无平台产物），此时**没有代码可捞，但必须照常评估**——在 `coding-analysis.md` 写明"无平台代码产物，题目走屏幕共享/口述"，据 asr 还原题面与候选人思路，按形态表定性（多为"只口述、没落代码"或"未取样（时间不足）"），**不脑补代码**。
...
- 编程题：**必须两路都核**。①`coding=yes` → 有 `candidate-code.<ext>` 与 `coding-analysis.md`；②**`coding=no` 时不得直接判"本轮未考"**——先回 `asr.md` 扫编程题信号（`算法题`/`看一个题`/`共享一下`/`编程题` 等，见 0.15 第 3 条），扫到则按"只口述、没落代码"形态写入 `coding-analysis.md` 与评价的「编程题」一节；扫不到才写"本轮未考"。
