...
9. **告警文案写给接收人,不写给维护者:出了什么事 / 影响谁 / 该干什么,一屏读完**。上面 7、8 两条管
   文案"准不准",这条管"读得懂不懂"。真实翻车:一条 description 塞进六个场景各自的阈值、定标日期与
   方法、两次上调阈值的理由、PromQL 归一化说明、五步排查法、三段历史事故,逾 600 字,用户的第一反应
   是"这个告警规则为什么这么长?应该简化一下";几天后另一条模板成功率告警被整段贴回"这个告警我看不懂,
   文案怎么改?"——它写的全是「指标怎么算」,没有一句「出了什么事、该干什么」;几条供应商错误率告警也都
   是被用户整段贴回来问"这个问题怎么做 / 如何止损"。规矩:
   - `summary` 一句话:什么坏了 + 用户可感吗。`description` 固定三段——**发生了什么**(现象、量级、
     持续多久)、**影响谁**(用户可见 / 内部降级 / 只是观测量变化)、**该干什么**(第一步看哪、止损动作
     是什么、由谁做)。总长以手机推送一屏为上限。
   - 阈值定标依据、历史上调理由、PromQL 归一化说明、排查步骤清单、历史事故——这些是维护者信息,放
     runbook / spec 里用一个链接引过来,或写进 spec 的 YAML 注释;不进推送文案。
   - 推送前自问一句「收到的人不开看板,照这段文字知道该不该动、动什么吗」,答不上来就重写。
   - 测试 / 演练用的告警同样简洁:一行说明这是测试、要接收人做什么(如回一个确认)即可。
   - 用户把告警整段贴回来问"这是什么问题 / 我该做什么",与盯守播报被贴回同理(见监控运维类 skill),
     是文案失败的实锤——修规则文案并重推,不是只解释这一条。
