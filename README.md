...
Tipsy 后端**全链路值班排障**（只读排查，不做变更）：4 个 MCP（Bytebase / SigNoz / aliyun-sls / Logfire）+ 3 条直连通道（Redis RunCommand OpenAPI / ES REST / memory 服务 curl）+ coolify CLI 串成一条排查流水线：
- 数据核对：MySQL / PostgreSQL / Lindorm / Redis / Elasticsearch 全通道，prod / test / preview 三环境显式隔离
- 日志与 trace：SLS（tipsy-backend Go）+ SigNoz（tipsy-memory Python）+ trace_id 三路拉齐
- 指标：ARMS Prometheus P99 / 错误率 / 熔断状态，内置 5 大 PromQL 陷阱防护
- 结论输出严格按「现象 / 通道 / 查询 / 结论 / 后续」五段格式
- 敏感配置（AK/SK / 实例 ID / endpoint / 内部 URL）全部走仓库外的 secrets 文件

安装：`./skills/tipsy-oncall/install.sh`（复制 skill + secrets 键追加 + MCP 体检；`--replace-legacy` 清理旧的项目级 tipsy-debug）。

使用示例：在 Claude Code 里说
> "查 prod 环境 session xxx 的 mempoint" / "angel 角色为什么从推荐位消失了" / "这个 trace_id 帮我全链路看一下"

使用说明：[skills/tipsy-oncall/README.md](skills/tipsy-oncall/README.md)
