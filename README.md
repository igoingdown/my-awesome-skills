...
- **判定**：`prompts/triage.md` 判 is_bug + 抽结构化字段（module / uid / cid / severity）
- **定位**：Step 7 先扫 `<github_root>/*` 得候选仓（`module=pay` 时不再默认只开主后端仓，独立 `-subscription` / `-billing` 之类微服务也会开），worktree 隔离 + 可选 Dynamic Workflow 并发跑 SLS / DB / memory / 代码 四路
- **实锤 5 档 rubric**：ground_truth / strong_signal / absence_signal / inference / hypothesis 分级；`confirmed` 门槛严——**现网数据 + 代码完整函数体已读 + 修复线索**三方 ground_truth 齐才算
- **对抗式自检**：每份报告强制列 3 条最弱证据 + 反命题能否成立 + 什么新数据能证伪 + 时区/单位 sanity check + 完整函数是否都读
- **存量 handoff**：`confirmed` 且存在存量待补时，自动锁定修复 commit author + 拉全量存量清单 + 生成飞书私信草稿交给用户 review（不自动发）
- **MCP 断线兜底**：连续 Stream closed 时主动切通路（Read → Bash heredoc / bytebase query → call_api / SLS 换 region），不盲目重试
- **子 agent 硬约束**：主 loop 前置贴现网数据快照，子 agent 只做代码理解 + 数值推演，不做数据获取（防"想当然"）

使用示例：在 Claude Code 里说
> "跑一轮 bug 三分类" / "把飞书 bug 群新消息挑一条给我 review" / `/loop 5m /bug-triage-loop`
