...
10. **写报告**：按 `references/report-templates.md` 的模板，用 `Write` 输出到 **`<人名>/<目标轮目录>/`**：
    ...
    - 若本轮有编程题：另写 `coding-analysis.md`（题面 + 审查清单 + 验证结果），代码原文存 `candidate-code.<ext>`。**有代码必先实跑再下结论。**
      - ⚠️ **但"实测"这个词只能用在补齐环境性缺失（import、main 入口、包声明）后仍是候选人算法的情形。** 代码缺核心控制流（return、目标值计算、递归入口、调用关系）时属形态 `C0`，补齐这些等于替他定义了算法主体——此时**没有实测结果，只有「审查性验证」**，且结论不作判分依据。禁用"实测/实跑/对跑"这类会被读成提交物运行记录的措辞。**真实事故 2026-09-11 两份报告同时踩**，详见 `references/scoring-rubric.md`「可验收主体」定义下的落笔规则。
