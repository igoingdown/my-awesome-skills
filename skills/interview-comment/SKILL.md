...
11. **多路独立 review（发布门禁，默认必做）**：报告写完后跑 `~/hire_patrol/multi-review.sh <人名>/<轮次目录>`，用 3 个模型 × 2 个侧重共 6 路独立复核。
...
12. **对话简报**：告知候选人综合分、通过/不通过、核心理由（长处/短处速判）；另附面试官评分和核心改进建议；multi-review 的结论与是否改判；最终给出所有产出文件路径
