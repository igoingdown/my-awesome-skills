...
while IFS= read -r d; do
  ...
  if [[ $ok_ev -eq 1 ]]; then
    ...
    # 模板必需标题缺失
    for h in '## 信息汇总' '### 概览' '### 维度依据' '### 优点' '### 待提升点' '### 风险点' '### 后续面试重点关注' '## 面试记录'; do
      grep -qF "$h" "$ev" || gate+="缺模板标题[${h}] "
    done

    # ---- 概览字数上限（事故 2026-09-11）----
    # 用户原话："预览部分每一个环节都写得太长了……概览部分最好不要超过 100 字，
    # 最长绝对不要超过 200 字"。实测三份报告的概览分别 2230/1669/1256 字，
    # 超上限 6-11 倍，单个维度段落最长 449 字——全篇最重要的一段反而最难读。
    # 根因是旧规则自相矛盾：既要"概览给只读一段的人看"，又要"六维依据都塞进来"。
    # 现已拆成「概览给结论 + 维度依据给证据」，这里机械守住概览的字数。
    # 口径：只数汉字（CJK 统一表意文字），不计标点、英文、数字、Markdown 标记——
    # 与用户说的"字"一致，也避免时间戳和代码标记把长度撑虚。
    ov_chars="$(sed -n '/^### 概览/,/^### 维度依据/p' "$ev" \
      | sed '1d;$d' \
      | python3 -c "import sys,re; print(len(re.findall(r'[一-鿿]', sys.stdin.read())))" 2>/dev/null || echo 0)"
    if [[ "$ov_chars" -gt 200 ]]; then
      gate+="概览${ov_chars}字超200上限 "
    fi

    # ---- 编码判"未考完"(C0) 时不得声称"实测"（事故 2026-09-11）----
    # C0 = 未形成可验收主体（骨架都没写：没有循环体、没有递归结构、只有函数签名）。
    # 此时不存在可运行的候选人代码，任何"实测"措辞都会被读成"他的代码跑失败了"。
    # 两份报告曾写成"忠实移植后实跑"+「实测结果」小节，一份还给出"195 组中 12 组判错"。
    #
    # ⚠️ 注意这一条只在维度表标注「未考完/未取样」时触发。若形态是 C2（骨架完整、
    # 只缺 return/递归入口这类收尾，补齐后能跑），维度表填 +/=/- ，本检查不触发——
    # C2 场景下如实写实测结果是正确做法（但要交代补了哪几处收尾）。
    # 同日曾把三份 C2 误判成 C0，就是因为把"缺收尾"当成了"缺骨架"。
    ...
    # ---- 编码判 - 或留空时，coding-analysis 必须列出终止序列（事故 2026-09-11）----
    # C0(留空) 与 C1(判 -) 的分界完全取决于"谁先宣告失败"，判错跨通过线。
    # 同一天两个相反方向各错一次：三份把"缺收尾"误判 C0(高估)，一份把"候选人
    # 自己说做不出"读成"面试官主动打断"(低估，综合分差一整档)。
    # 要求：报告里必须有带时间戳的终止段原话，逼作者回原文逐条核对而不是凭印象。
    if grep -qE '^\| *编码与算法 *\| *(-|（?(本轮)?未(考完|取样))' "$ev"; then
      ca="$d/coding-analysis.md"
      if [[ -s "$ca" ]]; then
        # 终止段证据：需同时出现"谁终止/先宣告"这类判定词，且带至少 2 个时间戳
        grep -qE '谁(先)?(宣告|终止)|先宣告失败|谁终止' "$ca" \
          || gate+="编码判-/留空但coding-analysis未交代谁先宣告失败 "
        ts_n="$(grep -oE '1?[0-9]:[0-9]{2}' "$ca" | sort -u | wc -l | tr -d ' ')"
        [[ "$ts_n" -lt 4 ]] && gate+="终止序列时间戳不足(${ts_n}个,需≥4) "
      fi
    fi
