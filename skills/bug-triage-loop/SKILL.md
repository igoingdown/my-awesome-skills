...
> **按需加载**：定位/取证/下结论（Step 7-8）与取数遇阻降级时，读 `references/investigation-discipline.md`——现状回验三查、扩样收手、召回验证、确认偏差、统计口径、各类降级取数与清理门禁等排查方法论都在那里，不必每次全读，用到哪块读哪块。
...
打分**前**必须过 `references/investigation-discipline.md` 的取证门禁——直接照 asr/单点证据下结论是本 skill 最常见的翻车源。按需读那份，核心门禁一句话预览（详规见该文件）：

- **现状回验三查**：问题现在还在吗（查最新状态）/ 用户行为时间线（对齐投诉点）/ 用户价值画像（order+subscription+wallet 三表）+ 实体血缘 + 群体恢复逐人核查。
- **扩样收手判据**：先分清缺的是样本量还是判定口径；连续扩样仍为零 → 出反向结论、别继续扩。
- **召回方法先自证**：主观体验类现象走语义判定，不靠关键词/正则；"扫了没命中"不算证据。
- **确认偏差门禁**：正反样本成对交付、带完整上下文、归属先证再用、叙事逐句挂证据；否则结论上限 hypothesis。
- **统计口径三件套**：每个比例/规模紧邻交代筛选条件、分子分母单位、集合关系 + 已知偏差；自造指标名就地定义。
...
### Step 12.6:根因落在"某客户端版本写坏了值"时,交付一张 trace 交接卡(P1 规则,来自一次真实事故复盘)

有一类 bug 的根因不在你能改的后端代码里,而是**某个特定客户端版本提交了一个非预期的值**(如某 iOS 版本把某上限字段写成异常大值)。这时用户要的不是"你去修",而是「把那次写入的 trace_id 捞出来,我让前端同学去查」——deliverable 是一张能直接转给**另一个团队**的自包含交接卡。要点:

1. **先从日志重建那次写入,锁定 trace_id**:按 uid + 精确时间窗(用户给的分秒)拉应用日志,client 侧过滤出那条写操作(full-text 搜 uid 会被别人响应体误中,见 `references/investigation-discipline.md` 的取数纪律),拿它的 `trace_id`。确认这条 trace **只**做了这次写入、没混入其它接口调用(核对它的接口边界),否则 trace 指向不纯。
2. **交接卡的核心三件就够,不要过度补字段**:`trace_id` + 精确时间戳(到毫秒/秒)+ 客户端标识(platform / app_version / client_ip)。**接收方拿这三件就能在他们自己的 APM / 埋点里下钻那次请求**——你这边日志展示层截断了 path/UA/req body 时,不必为把整行捞全去反复换 export / 换 query(观测到:为补一个对前端定位帮助有限的尾部字段折腾了三四轮)。判据:receiving 团队定位靠的是 trace_id,不是你日志里的那几个截断字段;三件齐了就收手交付,把"未取全的字段"如实标注,而不是无限深挖。
3. **归属写清"这是客户端行为,不是后端 bug"**:交接卡开头一句点明这条 trace 对应的是哪次客户端提交,后端只是忠实落库了客户端传来的值。别让接收方误以为要改后端。
...
> **下机制根因 / 判跨服务字段语义前，读 `references/investigation-discipline.md` 的对应门禁**（本段不再内联）：机制归因四道门禁（残差清单 / 对照实验 / 差分先于探针 / 领域边界，任一不过则机制结论 ≤ hypothesis）、跨服务同名字段边界门禁（先钉服务/表/库、来源标记≠跨服务写入、跨服务通道逐条枚举、快照时点语义）。
