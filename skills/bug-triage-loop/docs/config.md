...
**维护时机**:每次排查中发现新通路(或某通路失效),当场更新此表。**用户主动交付的新凭证/新开通的通路也算**(如一把新的云厂商 API key):当场入表并写明凭证所在的 secrets 变量名、能查什么数据、边界在哪,让并行 session 都能自取,不要让用户在每个 session 里重复粘贴同一段开通说明。

**通路被整体停用时**(账号回收 / 服务下线 / 用户宣布"以后都不要再用这条"):把该行标注 `已停用(YYYY-MM-DD)` 并补上替代通路,**不要只是本轮绕过**——留在表里的通路下一轮还会被照着试一遍。停用后的取数改走「SQL 写进云文档 → 用户代跑 → 结果回填 → 读文档取数」闭环,细则见 `references/investigation-discipline.md` 的对应一节。
