...
- **Switching one path to a new provider must not repoint a credential or config key that other paths still consume — enumerate every consumer of the key first, then give the migrating path its own independently named key.** A provider switch reads as "change the key and base URL", but secret keys and env vars are shared surfaces: changing the value of an existing key silently re-routes every other path that reads it. Observed, one day apart: a key swap meant for one pipeline flipped an unrelated experiment's traffic to the new vendor and was only caught the next day ("yesterday's key change affected the other experiment"); the redesigned wiring that populated one path's variable by referencing another path's key drew the blunt correction "changing one key can affect another — that's wrong"; and even the final version got "these keys are still coupled — fix it". The discipline, in order: (1) before touching any key, list every consumer of it — online main path, side paths, scheduled jobs, one-off scripts — from the manifests and code, not from memory; (2) the migrating path gets a **new key with its own name** whose value only it reads, and the old key's value is left untouched, so "change A" provably cannot move B; (3) in manifests, inject same-name (env var name equals the secret/config key name) instead of aliasing one name onto another — an alias layer hides the blast radius of a key change and is exactly where the coupling hides; (4) keep switch-type knobs (which provider a path routes to) in the central config store, not scattered across per-deployment envs, so the current routing is readable in one place; (5) before changing how a whole bundle is injected (e.g. moving to whole-bundle env injection), check which other services and scheduled jobs in the same cluster consume that bundle. After the switch, re-check for residual coupling yourself — "these keys are still coupled" is a review the user should not have to open.
- **Before scaling a batch onto a new provider key, probe its safe concurrency instead of trusting the advertised limit — then ramp, don't jump.** Switching a batch to a fresh key/endpoint (a different vendor, a higher-tier account) to "go faster" only pays off if you find the concurrency that key sustains without 429s, and the advertised RPM is an upper bound, not a safe operating point. Measure it first: fire a short burst of representative requests at rising concurrency against that key and watch where 429s start, then set the run's concurrency comfortably below that knee. Don't jump straight to a large concurrency because the new key "allows more" — ramp in steps (observed: an operator moved a scan to a higher-RPM key and wanted concurrency at 100; the safer call was to step to 30 first, confirm zero 429s, then climb). Make the concurrency a validated knob (see the parse-time-limits bullet) and, if the key is shared with anything live, isolate it first (see the shared-resource bullet). A run that trips sustained 429s is slower than a smaller run that never backs off.
- **When per-item model calls hang or error, fix the request shape before blaming the channel.** A per-item model-call batch (LLM-as-judge over large material, per-row rewrite, extraction) that stalls or fails is not evidence the provider or channel is down — two distinct failure modes both look like "the model calls hang," and both are cured by changing the request, not the provider. (1) A **streaming** call with long silent gaps gets cut by a **stream idle-timeout**: the connection is healthy, the upstream just goes quiet while composing a large answer — switch that call to **non-streaming** with a generous overall timeout. (2) A single **oversized prompt** trips the upstream's **hard request timeout** (a gateway 5xx) regardless of streaming — shrink the per-request payload (fewer items per call), keeping total work the same across more calls; observed that a large per-call bundle timed out while the same bundle split small returned in seconds. Prove which one it is before swapping models: a **small probe request on the same channel** that returns promptly means the channel is alive and the fault is request shape. Swapping model or family only appears to help when the substitute happens to route through a different upstream — that masks the real cause and the problem returns on the next large call.
