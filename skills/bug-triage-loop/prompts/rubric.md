...
- ❌ **"SLS 搜引号短语命中 N 条 = 该报错发生了 N 次"** → 直接当 strong_signal / 直接进结论。**正确**: 分词型日志库对引号短语做的是切词 AND 而非短语匹配, 长英文 payload 极易凑齐普通词。命中数必须用 `content like '%原短语%'` 二次过滤得到 exact 后才能引用, 报告写 `exact=N (tokenized=M)`; 只有 tokenized 的数字权重上限按 absence_signal 算。**这个规则来自一次把 0 条真实报错报成 P0 主链路故障的真实事故**(1700+ 分词命中 / 0 精确命中), 且同坑两个月内复犯 ≥7 次——见 `references/investigation-discipline.md`「分词型日志库的关键词计数门禁」
