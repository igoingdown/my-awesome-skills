...
> **Note**:下面第 1 / 2 节以作者用的技术栈(阿里云 SLS / Lindorm / DMS + Bytebase + Coolify + Grafana + Logfire + SigNoz + lark-cli)为例,给出**具体命令 / 环境变量名**方便你照抄骨架。你的项目使用其他 SaaS 时把对应的名字替换即可,skill 骨架 / 规则 / 精度硬约束不变。
...
- 项目侧 oncall / debug skill(如 `<project>-oncall` / `<project>-debug`)—— 封装了 SLS/DMS/Lindorm/Redis 查询和"现象/通道/查询/结论/后续"输出格式
...
│   ├── 拉起 bytebase MCP server 子进程   → 读 <PROJECT>_BYTEBASE_URL
