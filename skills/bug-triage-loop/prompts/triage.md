...
2. **技术名词 逐个消歧**:
   ...
   - **一个词碰巧和某个后端子系统同名, 不等于问题就在那个子系统**(来自一次真实事故复盘): 某项异常"上限/长度/配额"数值, 名字里带某子系统的字眼(如叫"XX 记忆上限"), AI 顺着字面把它当成该子系统(记忆/缓存/检索层)的内部故障去查——但结合代码看, 那其实是**用户或客户端可自行设定的一个普通配置字段**, 跟那个子系统的存储和链路毫无关系。这个消歧错一次, 排查方向和最终归属(是产品配置/客户端写入, 还是后端子系统 bug)整个偏掉。用户的纠偏形态是「这是用户可以设定的 X, 你结合代码看清楚, 这跟 Y 系统没有关系」。做法: 遇到"名字像某子系统的数值型字段", 先回代码确认它的**写入方**(用户设置接口 / 客户端提交 / 系统默认 / 该子系统内部计算)和存储位置, 再决定往哪个方向查; 别被字面命名带跑。
...
4. **原话 vs 转述失真校验** (来自一次"AI 替用户说话"真实事故复盘):
   - CS/运营转发的中文标题是**转述**, 常引入用户没说过的问题 (例: 用户原话 "my chat answers for me and can't stop it" 被转述为"角色回复过长并且无法停止" — "回复过长"是转述者加的, 用户从没抱怨过长度; "can't stop it" 指的是"停不掉代述行为"而非"回复长得停不下来")
   - 判定与后续定位必须以**附件截图里用户原话 (verbatim)** 为准; 转述与原话不一致时, 在 extracted 里加 `original_complaint` 字段记录原话, title 修正为对准原话的表述, 并显式标注差异
   - 原话里的模糊指代 ("it"/"this") 要锚定指代物: 停不掉/坏掉的到底是什么行为? 用原话上下文或该用户的后续操作数据佐证, 不许沿转述的理解直接开查
